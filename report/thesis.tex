...
with a basic case with an horizontal line but that does cross a star or bright object.
\begin{figure}[h]
    \begin{minipage}[c]{.47\linewidth}
        \centering
...
is truly bright in comparison of other streaks that can be pretty discrete and even sometimes they can be invisible to the naked eyes.
Obviously, using the pipeline on this kind of tracks will give an imprecise result with a way bigger error percent.
\newline
\newline
Now let's see the result with a more realistic rotated line that does not cross any specially bright object that could introduce some 
...
\end{figure}
\newline
We can see that the frequency of the fitted function seems to approximate reasonably well the data. The pipeline finds $0.009962$ for the
...
PredicSat that also compute information about it, like the altitude and %%%% VOIR A MODIFIER

%%%%%%%%%%%%%%%%%%%%
\chapter{Conclusion}
%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%PARLER QUE DANS LA VRAIE VIE CEST PAS SINUSOIDAL
